...
export default class MarkwhenSidebarPlugin extends Plugin {
    ...
    public reinitializeHandlers() {
        ...
        this.registerMarkwhenClickHandlers(); // Всегда пытаться переинициализировать
    }
    ...
    private async activateView() {
        ...
        setTimeout(() => {
            this.registerMarkwhenClickHandlers();
        }, 300); // 300ms задержка для полной инициализации UI
    }
    
    // В методе registerMarkwhenClickHandlers замените текущую реализацию на:
    private registerMarkwhenClickHandlers() {
        ...
        if (iframeContainer && iframeContainer.contentWindow) {
            const tryAttach = () => {
                try {
                    const iframeDoc = iframeContainer.contentDocument;
                    if (iframeDoc && iframeDoc.readyState === 'complete') {
                        iframeDoc.addEventListener('click', this.handleIframeClick.bind(this));
            ...
            // Пытаемся сразу прикрепить обработчик если iframe уже загружен
            if (tryAttach()) return;

            // Добавляем обработчик на событие загрузки
            const loadHandler = () => {
                iframeContainer.removeEventListener('load', loadHandler);
                tryAttach();
            };
            iframeContainer.addEventListener('load', loadHandler);

            // Фолбек на случай если событие load не сработает
            setTimeout(() => {
                if (!this.isHandlerRegistered) tryAttach();
            }, 1000);
        }
    }

    // В методе startConnectionMonitor уменьшите интервал проверки:
    private startConnectionMonitor() {
        const monitorInterval = setInterval(() => {
            if (!this.isHandlerRegistered) {
                this.registerMarkwhenClickHandlers();
            }
        }, 1000); // Уменьшено с 5000 до 1000 мс
